...
namespace JSG.WordPalace
{
    public class UISystem : MonoBehaviour
    {
        ...
        public UIGraphicContents m_Graphics;
        [HideInInspector]
        ...
        //public SquadCustomize[] m_SquadSettings;
        ...
        void Start()
        {
            ...
            m_GeneralCanvasSize = new Vector2(ratio * 1600, 1600);
        ...
        public static UIMessage_A ShowMessage(string msgName, int msgType, string msgText, Sprite msgIcon)
        {
            GameObject uiprefab = FindUIByName(msgName);
            if (uiprefab != null)
            {
                GameObject uiObj = Instantiate(uiprefab);
                uiObj.transform.SetParent(m_Main.transform);
                ...
                m_Main.m_LastUI = uiObj;
                m_Main.m_UIStack.Add(uiObj);
                UIMessage_A msg = uiObj.GetComponent<UIMessage_A>();
                msg.m_MessageText.text = msgText;
                msg.m_MessageImage.sprite = msgIcon;
                msg.m_MessageType = msgType;

                Canvas canvas = uiObj.GetComponentInChildren<Canvas>();
                if (canvas != null)
                {
                    canvas.sortingOrder = m_Main.m_MessageLayerOrder;
                    m_Main.m_MessageLayerOrder++;
                }
